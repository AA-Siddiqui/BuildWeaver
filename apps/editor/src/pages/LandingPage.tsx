    ...
    <section className="relative overflow-hidden bg-gradient-to-br from-bw-crimson via-bw-clay to-bw-ink py-20 text-center">
      ...
      <div className="bw-container relative flex flex-col items-center gap-8">
        ...
        <p className="max-w-2xl text-base text-bw-platinum/80 md:text-lg">
          BuildWeaver unifies UI, logic, data, and LLM assistance so teams can go from idea to runnable code in a single, auditable workflow.
        </p>
        <div className="flex flex-col gap-4 sm:flex-row">
          <Link
            to="/workspace"
            className="rounded-full bg-bw-sand px-8 py-3 text-sm font-semibold uppercase tracking-wide text-bw-ink shadow-glow-sm transition hover:-translate-y-1"
          >
            Go to Workspace
          </Link>
          <Link
            to="/login"
            className="rounded-full border border-white/40 px-8 py-3 text-sm font-semibold uppercase tracking-wide text-white/90 transition hover:bg-white/10"
          >
            Login to continue
          </Link>
        </div>
        ...
        {features.map((feature) => (
          ...
          >
            <div className="mb-4 h-12 w-12 rounded-2xl bg-gradient-to-br from-bw-crimson to-bw-amber p-[2px]">
              <div className="flex h-full w-full items-center justify-center rounded-[1rem] bg-bw-ink text-xl font-bold text-bw-sand">
                ✦
              </div>
            </div>
